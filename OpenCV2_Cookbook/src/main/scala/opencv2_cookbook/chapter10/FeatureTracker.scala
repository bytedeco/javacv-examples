...
import org.bytedeco.javacpp.{FloatPointer, BytePointer, IntPointer}
...
class FeatureTracker(maxCount: Int = 500,
  ...
  def process(frame: IplImage): IplImage = {
    ...
    val trackingStatus = new BytePointer(trackedPoints.length)
    cvCalcOpticalFlowPyrLK(
      ...
      cvSize(21, 21), 3, // Defaults
      trackingStatus, // tracking success
      new FloatPointer(trackedPoints.length), // Not used
    ...
    for (i <- 0 until trackedPointsNewUnfiltered.size) {
      if (acceptTrackedPoint(trackingStatus.get(i), trackedPoints(i), trackedPointsNewUnfiltered(i))) {
  ...
  def detectFeaturePoints(grayFrame: IplImage): Array[CvPoint2D32f] = {
    ...
    val featureCount = new IntPointer(1)
    ...
    toArray(featurePoints).take(featureCount.get(0))
  }
