...
import org.bytedeco.javacpp.opencv_legacy._
...
class Histogram1D {
  ...
  def getHistogram(image: IplImage, mask: IplImage = null): CvHistogram = {
    ...
    val hist = imgproc.cvCreateHist(dims, sizes, histType, ranges, 1)

    // Compute histogram
    val accumulate = 0
    imgproc.cvCalcHist(Array(image), hist, accumulate, mask)
    hist
  }
